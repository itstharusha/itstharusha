# Tharusha Rathnamalala

## About Me
I am a **Data Science undergraduate** specializing in **Artificial intelligence, machine learning, and computer vision**.  
I develop **scalable, production-ready systems**, implement **data-driven solutions**, and focus on **clean, maintainable code**.  

I am actively seeking **AI engineering or machine learning internships/full-time roles** where I can contribute to **high-impact projects** and grow as a technical leader.

---

## Core Competencies

**Programming Languages:** Python, Java, C, R  
**Machine Learning & AI:** Supervised & Unsupervised Learning, Deep Learning (TensorFlow, PyTorch), ML Model Deployment, MLflow, Scikit-learn  
**Computer Vision:** OpenCV, Image & Video Processing, Object Detection, Classification, Segmentation  
**Data & Databases:** SQL (MySQL, SQL Server, SQLite), ETL Pipelines, Data Cleaning & Feature Engineering, Pandas, NumPy  
**Cloud & DevOps:** AWS, Azure, Firebase, Docker, Kubernetes, CI/CD, Vercel  
**Data Visualization & Analytics:** Matplotlib, Plotly, Power BI, Dashboard Development  
**Tools & Collaboration:** Git, GitHub, Jupyter Notebooks, Postman, OpenAPI  
**Design & UI:** Adobe Illustrator, Adobe Photoshop  

---

## Selected Projects

**End-to-End ML Pipelines**  
- Developed and deployed predictive models using Python, TensorFlow, and PyTorch.  
- Implemented data preprocessing, feature engineering, model training, and evaluation.  
- Integrated models with APIs for real-time usage.

**Computer Vision Applications**  
- Built object detection and image classification pipelines using OpenCV and PyTorch.  
- Optimized models for inference speed and accuracy on large datasets.  

**Data Analytics Dashboards**  
- Created dashboards in Power BI and Python for actionable insights.  
- Automated data collection, cleaning, and visualization for business use cases.  

*(Add project links or GitHub repos for each project.)*

---

## GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=itstharusha&show_icons=true&theme=dark&hide_border=false&include_all_commits=true&count_private=true)  
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=itstharusha&layout=compact&theme=dark&hide_border=false)

---

## Contact
- **LinkedIn:** [linkedin.com/in/TharushaRathnamalala](https://linkedin.com/in/TharushaRathnamalala)  
- **GitHub:** [github.com/itstharusha](https://github.com/itstharusha)  
- **Email:** itstharusha@gmail.com  

---
